	...
	public static int[] getSpanningTree(NetNode from){
		...
		for(int i = 0; i<archi.length; i++){
			...
			for(int j=0; j<nodi.length; j++){
				if(nodi[j].equals(inizio)){id1=j;}
				else if(nodi[j].equals(fine)){id2=j;}
			}
...
//		System.out.println(archiDaEliminare);
		...
		a: for(Edge e : reteVera.getEachEdge()){
			...
			for(NetEdge arco : archiDaEliminare){
				...
				if(nuovoArco.getSourceNode().getId().equals(arco.getTargetNode().getId()) && nuovoArco.getTargetNode().getId().equals(arco.getSourceNode().getId())
...
//			System.out.println("pre    "+Arrays.toString(pred));
	...
	static class Grafo {
		...
		public Grafo(Arco[] edges){
			...
			/*TODO*
			System.out.println("grafo .costr. ");
			for(int i=0; i<this.edges.length; i++)System.out.print("-"+this.edges[i]);
			System.out.println();
			System.out.println("#edges="+edges.length+" #nodes"+nodes.length);
			for(int i=0; i<this.nodes.length; i++)System.out.print("-"+nodes[i]);
			System.out.println();
			/**/ 
