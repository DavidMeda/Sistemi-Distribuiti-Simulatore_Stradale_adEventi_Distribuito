...
public class Statistiche extends UnicastRemoteObject implements ServerStatistiche {
	...
	private String datiFinali, datiVeicoli,statisticheFinali;
	// variabili RSU
	private String idRSUMessMax, idRSUMessMin, idRSUGradoOttMin, idRSUGradoOttMax;
	private int RSUtot = 0, numMessMassimo = Integer.MIN_VALUE, numMessMinimo = Integer.MAX_VALUE, sommaMessTotali = 0, sommaMessInviatiRSU_RSU = 0, sommaMessRicevutiRSU_RSU = 0,
					sommaMessInviatiRSU_Veicoli = 0, sommaMessRicevutiRSU_Veicoli = 0, mediaPersorsiMinimi = 0, mediaPercorsiTot = 0, mediaMessRicevuti = 0, mediaMessInviati = 0,
					mediaMessTot = 0;
	private double sommaGradoOttimalitaPercorsiMinimi = 0.0, sommaGradoCongestione = 0.0, mediaGradoCongestione = 0.0, mediaGradoOttimalitaPercorsiMinimi = 0.0,
					mediaDurataTotale = 0.0, durataTotale = 0.0, gradoOttMin = Double.MAX_VALUE, gradoOttMax = Double.MIN_VALUE;
	// variabili veicoli
	...
	private void calcolaStatisticheGenerali() throws RemoteException {
		...
		sb.append("\n\t- Veicolo con il minor tempo di attesa: " + idVeicoloAttesaMinima + " [" + tempoDiAttesaMinimo + " sec]");
		sb.append("\n\t- Veicolo con il maggior tempo di attesa: " + idVeicoloAttesaMassima + " [" + tempoDiAttesaMassimo + " sec]");
		...
		sb.append("\n\t- RSU col minor grado di ottimalità sui percorsi minimi = " + idRSUGradoOttMin + " (num: " + gradoOttMin + ")");
		sb.append("\n\t- RSU col maggior grado di ottimalità sui percorsi minimi = " + idRSUGradoOttMax + " (num: " + gradoOttMax + ")\n");
		
		datiFinali = sb.toString();
		...
		datiVeicoli =sb.toString();
		for (Entry<RemoteRSU, VariabileStatRSU> e : mappaStatistiche.entrySet()){
			e.getKey().stampaStatistiche(datiFinali);
		}
		displayText(datiFinali, datiVeicoli);
	}

	private void displayText(String statisticheGen, String statisticheVeicoli) {
			...
			public void actionPerformed(ActionEvent e) {
				...
				if(nomeFile != null){
					try{
						...
						salvaStatistiche(statisticheFinali, nomeFile);
		...
		textArea.append(statisticheGen);
	}
