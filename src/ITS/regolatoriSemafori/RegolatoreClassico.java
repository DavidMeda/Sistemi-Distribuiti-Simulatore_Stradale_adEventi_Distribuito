...
public class RegolatoreClassico extends ABS_Regolatore {
	...
	public void init() {
		...
		for(Entry<NetNode, ArrayList<Semaforo>> e : semafori.entrySet()){
			s = e.getValue().get(0);
			ls = new LinkedList<>(); ls.add(s);
			fasi.add(new Fase(s));
		}
		
		/*print*
		System.out.println(this+": init.");
		System.out.println("-- fasi= "+fasi);
		/**/
		setFasi(fasi);

		//inizia il ciclo delle fasi
//		Message start = new Message("CAMBIO FASE", sourceRSU, sourceRSU, 0);//posso inviarlo direttamente all'rsu TODO provare 
